...
def manage_users():
    ...
    return render_template('pages/admin/users/users.html', users=users)
...
def add_user():
    ...
    if form.validate_on_submit():
        if not form.password.data:
            ...
            return render_template('pages/admin/users/add_edit_user.html', form=form, title='Thêm Người Dùng Mới')
...
def edit_user(user_id):
    ...
    if form.validate_on_submit():
        ...
        if form.password.data:
            if form.password.data != form.password2.data:
                form.password2.errors.append('Mật khẩu không khớp.')
                return render_template('pages/admin/users/add_edit_user.html', form=form, title='Sửa Người Dùng', user=user)
            user.set_password(form.password.data)
