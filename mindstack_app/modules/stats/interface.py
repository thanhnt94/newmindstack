...
class StatsInterface:
    ...
    @staticmethod
    def get_vocab_set_overview_stats(user_id: int, set_id: int, page: int = 1, per_page: int = 12, sort_by: str = 'default', filter_mode: str = 'all', q: str = None) -> dict:
        """Get overview statistics for a vocabulary set."""
        from .services.vocabulary_stats_service import VocabularyStatsService
        return VocabularyStatsService.get_course_overview_stats(user_id, set_id, page, per_page, sort_by=sort_by, filter_mode=filter_mode, q=q)
    ...
    def get_user_activity_heatmap(user_id: int, weeks: int = 12) -> list:
        ...
        return get_user_activity_heatmap(user_id, weeks)

    @staticmethod
    def get_difficult_items_overview(user_id: int, limit: int = 10) -> list:
        """Get top difficult items."""
        return get_difficult_items_overview(user_id, limit)
