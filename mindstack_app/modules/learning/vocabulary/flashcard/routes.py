# File: vocabulary/flashcard/routes.py
# Phiên bản: 4.0 - Integrated into Vocabulary Hub
# MỤC ĐÍCH: Flashcard learning routes within vocabulary context

from flask import render_template, request, jsonify, abort, current_app, redirect, url_for, flash, session
from flask_login import login_required, current_user
import traceback
from . import vocab_flashcard_bp
...
@vocab_flashcard_bp.route('/get_flashcard_options_partial/<set_identifier>', methods=['GET'])
@login_required
def get_flashcard_options_partial(set_identifier):
    ...
    return render_template('flashcard/individual/setup/_modes_list.html',
                           modes=modes,
...
@vocab_flashcard_bp.route('/start_flashcard_session/all/<string:mode>', methods=['GET'])
@login_required
...
@vocab_flashcard_bp.route('/get_flashcard_batch', methods=['GET'])
@login_required
...
@vocab_flashcard_bp.route('/flashcard_learning/api/items/<int:item_id>', methods=['GET'])
@login_required
...
@vocab_flashcard_bp.route('/submit_flashcard_answer', methods=['POST'])
@login_required
...
@vocab_flashcard_bp.route('/end_session_flashcard', methods=['POST'])
@login_required
...
@vocab_flashcard_bp.route('/save_flashcard_settings', methods=['POST'])
@login_required
...
@vocab_flashcard_bp.route('/get_flashcard_sets_partial', methods=['GET'])
@login_required
def get_flashcard_sets_partial():
    ...
    try:
        ...
        return render_template('flashcard/individual/setup/_sets_list.html', **template_vars)
...
@vocab_flashcard_bp.route('/toggle_archive_flashcard/<int:set_id>', methods=['POST'])
@login_required
...
@vocab_flashcard_bp.route('/bulk_unarchive_flashcard', methods=['POST'])
@login_required
...
@vocab_flashcard_bp.route('/generate-image-from-content', methods=['POST'])
@login_required
...
@vocab_flashcard_bp.route('/regenerate-audio-from-content', methods=['POST'])
@login_required
