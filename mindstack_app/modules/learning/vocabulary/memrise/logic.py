...
def generate_question_payload(user_id: int, item: LearningItem, progress: FlashcardProgress = None):
    ...
    if progress.last_reviewed and progress.interval:
         quest['retention_rate'] = VocabularySrsService.calculate_retention_rate(
...
def process_answer(user_id: int, item_id: int, type: str, answer: str):
    ...
    if type == 'learn':
        ...
        VocabularySrsService.process_interaction(user_id, item_id, 'flashcard', {'quality': 4})
    ...
    srs_res = VocabularySrsService.process_interaction(
...
def get_course_overview_stats(user_id: int, container_id: int, page: int = 1, per_page: int = 20):
    ...
    for item, p in pagination.items:
        ...
        if p and p.last_reviewed and p.interval:
            retention = VocabularySrsService.calculate_retention_rate(p.last_reviewed, p.interval)
        elif status == 'new':
            retention = 0 
        else:
            retention = 0
