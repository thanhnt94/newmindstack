# File: Mindstack/web/mindstack_app/modules/my_content/courses/routes.py
# Version: 1.0 - Routes và logic cho quản lý Khóa học của người dùng
# Mục đích: Xử lý các yêu cầu liên quan đến việc tạo, sửa, xóa Khóa học và Bài học.
...
def list_courses():
    # Lấy tất cả các Khóa học do người dùng hiện tại tạo
...
def edit_course(set_id):
    ...
    # Đảm bảo người dùng hiện tại là người tạo Khóa học này
    if course.creator_user_id != current_user.user_id:
...
def list_lessons(set_id):
    ...
    # Lấy các bài học thuộc khóa học này
...
def add_lesson(set_id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('courses.list_lessons', set_id=set_id))
...
def edit_lesson(set_id, item_id):
    ...
    # Đảm bảo bài học thuộc khóa học và người dùng là người tạo
    if lesson.container_id != set_id or course.creator_user_id != current_user.user_id:
