"""
Typing Service - Orchestrates Database access and calls Typing Engine.
"""
...
class TypingService:
    ...
    def ensure_audio_urls(item, container: 'LearningContainer' = None, q_key: str = None, a_key: str = None) -> dict:
        ...
        def _gen(text: str, filename: str, target_dir: str):
            ...
            try:
                ...
                current_app.logger.warning(f"Failed to generate audio in Typing: {e}")
                return None
    ...
    def get_eligible_items(container_id: int, user_id: int = None) -> list:
        """
        Get processed items ready for Typing engine.
    ...
    def generate_session_questions(container_id: int, config: dict, user_id: int = None) -> list:
        ...
        # Merge config with container settings
        typing_settings = (container.settings or {}).get('typing', {}) if container else {}
        
        merged_config = {
            'mode': config.get('mode') or typing_settings.get('mode', 'front_back'),
            'question_key': config.get('question_key') or typing_settings.get('question_key'),
            'answer_key': config.get('answer_key') or typing_settings.get('answer_key'),
            'custom_pairs': config.get('custom_pairs') or typing_settings.get('pairs') or typing_settings.get('custom_pairs'),
        ...
        for item in selected_items:
            updated_content = TypingService.ensure_audio_urls(
                item, 
                container, 
    ...
    @staticmethod
    def check_result(user_input: str, correct_answer: str) -> dict:
        """Call engine to validate answer."""
        return TypingEngine.validate_answer(user_input, correct_answer)
