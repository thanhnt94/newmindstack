...
def mcq_api_next_question(set_id):
    ...
    if not success and manager.params.get('count') == 0:
        if manager.start_next_cycle():
            ...
            # Ensure the first question of the new cycle is generated
            manager.ensure_question_generated(0)
            return jsonify({
                'success': True, 
                'currentIndex': 0,
                'new_cycle': True,
                'questions': manager.questions,
                'currentQuestion': manager.questions[0]
            })

    # Return the current question data (fully generated by manager.next_item)
    current_q = None
    if manager.currentIndex < len(manager.questions):
        # Double check generation (safety)
        manager.ensure_question_generated(manager.currentIndex)
        current_q = manager.questions[manager.currentIndex]
