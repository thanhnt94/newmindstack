...
function playAudioForButton(button, options = {}) {
    ...
    // If audio source exists, try to load it first to verify it works
    if (hasAudioSource) {
        return new Promise((resolve) => {
            // Check if audio is already known to be broken (error state)
            if (audioPlayer.error || audioPlayer.networkState === HTMLMediaElement.NETWORK_NO_SOURCE) {
                // Audio source is broken, regenerate via TTS
                console.log('[Audio] Source broken, regenerating via TTS...');
                stopAllFlashcardAudio(audioPlayer);
                generateAndPlayAudio(button, audioPlayer, { awaitCompletion, suppressLoadingUi, restart }).then(resolve);
                return;
            }

            // Try to play existing audio
            stopAllFlashcardAudio(audioPlayer);
            FlashcardAudioController.playAudioAfterLoad(audioPlayer, { restart, awaitCompletion })
                .then(resolve)
                .catch((err) => {
                    // If play fails, try regenerating
                    console.log('[Audio] Play failed, regenerating via TTS...', err);
                    generateAndPlayAudio(button, audioPlayer, { awaitCompletion, suppressLoadingUi, restart }).then(resolve);
                });
        });
    }

    // No audio source, generate via TTS
    stopAllFlashcardAudio(audioPlayer);
