    ...
    return {
        ...
        playAudioAfterLoad(audioPlayer, { restart = true, awaitCompletion = false } = {}) {
            return new Promise(resolve => {
                ...
                // Check if audio is already in error state (e.g., 404)
                if (audioPlayer.error || audioPlayer.networkState === HTMLMediaElement.NETWORK_NO_SOURCE) {
                    console.log('[AUDIO] Audio source unavailable, skipping playback');
                    resolve();
                    return;
                }
                ...
                const onError = () => {
                    console.log('[AUDIO] Audio load error, skipping playback');
                    cleanup();
                    resolve();
                };
                ...
                // Always add event listeners for proper cleanup
...
// Setup audio error handler for fallback/retry logic
function setupAudioErrorHandler(itemId, frontAudioContent, backAudioContent) {
    // Error handling for audio elements - regenerate if initial source fails
    document.querySelectorAll('.js-flashcard-card audio').forEach(audio => {
        audio.addEventListener('error', async (e) => {
            ...
            const button = target.closest('.card-side')?.querySelector('.play-audio-btn');
            if (button && typeof generateAndPlayAudio === 'function') {
                console.log('[Audio] Marking audio for regeneration on error...');
            }
        });
    });
}
